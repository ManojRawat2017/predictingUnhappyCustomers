...
data = pd.read_csv("/data/DetractorPrediction/NPSPredictiveModelAttributesTenant_Split_2.csv",sep=',', header=None, names=Col_s, index_col=False)

Fields_to_check = ['TenantId' ,#'Name'
                  'TotalUsers'#,'LatestSATType','CompositeSATType','NumberOfTickets_Past90Days','Region'
                  ]

data_check = data.loc[:,Fields_to_check]

data_check.columns = ['OriginalId','Seats']

print(data_check.head())

Detractors = pd.read_csv('/data/DetractorPrediction/TenantDetractors_Split2.csv')

print(Detractors.head())

detractor_data = data_check[data_check.OriginalId.isin(Detractors.TenantId)]

#sample_from_detractors = data_check.merge(Detractors,left_on='OriginalId',right_on='TenantId',how='outer')

sample_from_detractors[:100].to_excel('/data/DetractorPrediction/OriginalData_withPredictions.xlsx')
#sample_from_detractors[:100].to_excel('/data/DetractorPrediction/OriginalData_withPredictions.xlsx')