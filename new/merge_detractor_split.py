...
def merge_detractor_split(split_num,csv_or_tsv='csv'):
    ...
    if csv_or_tsv == 'tsv':
        dta = pd.read_csv("NPSPredictiveModelAttributesTenant_Split_"+str(split_num)+".tsv" ,sep='\t',  #lineterminator='\n', 
                          names=Col_s )
    else:
        dta = pd.read_csv("NPSPredictiveModelAttributesTenant_Split_"+str(split_num)+".csv" ,#sep='\t',  lineterminator='\n', 
                          names=Col_s )
    All_Detractors = pd.read_csv('DetractorID.csv')
    dta = dta[dta.TenantId.isin(All_Detractors.TenantId)]
    dta.to_csv('FullDetractorData_Split'+str(split_num)+'.csv',index=False)

merge_detractor_split(5)